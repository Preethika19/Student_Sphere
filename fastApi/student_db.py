...
def save_student(student : Student , connection) :
    try :
        ...
        print(e)
        return "Error while saving. Retry !! "+str(e)
...
def updateStudentAddress(student_id : int, door_no: str, street : str, city : str, connection) :
    try:
        ...
        if result == 0 :
            return { "Error" : "No record found in student table with student id : " + str(student_id) }
        sql = "UPDATE STUDENT SET door_no=:door_no , street=:street, city=:city WHERE student_id=:student_id"
        ...
        cursor.execute(sql, param)
        connection.commit()
        return { "stauts" : "sucess", "message" : "Student with student_id " + str(student_id) + " updated successfully."  }
    except Exception as ex :
        print(ex)
        return { "Error" : ex }
